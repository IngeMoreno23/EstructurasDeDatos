...
template <class T>
HDCLinkedList<T>::Iterator::Iterator(NodeC<T>* currentNode, NodeC<T>* headNode) 
    : current(currentNode), head(headNode) {}

template <class T>
T& HDCLinkedList<T>::Iterator::operator*() {
    return static_cast<NodeC<T>*>(current)->getData();
}

template <class T>
class HDCLinkedList<T>::Iterator& HDCLinkedList<T>::Iterator::operator++() {
    current = static_cast<NodeC<T>*>(current->getNext());
    return *this;
}
...
template <class T>
class HDCLinkedList<T>::Iterator& HDCLinkedList<T>::Iterator::operator--() {
    current = static_cast<NodeC<T>*>(current->getPrevious());
    return *this;
}

template <class T>
bool HDCLinkedList<T>::Iterator::operator!=(const Iterator& otro) const {
    return current != otro.current;
}

template <class T>
class HDCLinkedList<T>::Iterator HDCLinkedList<T>::begin() {
    return Iterator(static_cast<NodeC<T>*>(this->head), static_cast<NodeC<T>*>(this->head));
}

template <class T>
class HDCLinkedList<T>::Iterator HDCLinkedList<T>::end() {
    return Iterator(static_cast<NodeC<T>*>(this->head), static_cast<NodeC<T>*>(this->head));
}